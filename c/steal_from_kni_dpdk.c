...
static void
print_ipv6(struct ipv6_hdr *v6) {
	int i;
	printf("vtc_flow, pl_len, proto, hop_lim %u, %u, %u, %u\n", v6->vtc_flow, v6->payload_len, v6->proto, v6->hop_limits);
	printf("src:");
	for (i = 0;i < 15;i ++) {
		printf("%x:", v6->src_addr[i]);
	}
	printf("%x, dst: ", v6->src_addr[15]);
	for (i = 0;i < 15;i ++) {
		printf("%x:", v6->dst_addr[i]);
	}
	printf("%x\n", v6->dst_addr[15]);
}
...
static int
check_steal(struct rte_mbuf *pkt, uint8_t port_id) {
	struct ether_hdr *eth_h;
	uint16_t eth_type, l2_len;
	...
	printf("eth_type is 0x%x, %u\n", eth_type, port_id);
	ip6_h = (struct ipv6_hdr*)((char*)eth_h + l2_len);
	if (ip6_h->proto != 17) {
		printf("ipv6 proto is %x\n", ip6_h->proto);
		return -1;
	}
	struct udp_hdr *u_hdr;
	u_hdr = (struct udp_hdr*)((rte_pktmbuf_mtod(pkt, char*) + l2_len + sizeof(struct ipv6_hdr)));
	// dns
	if (rte_be_to_cpu_16(u_hdr->dst_port) != 53) {
		printf("port is %x\n", u_hdr->dst_port);
		return -1;
	}
...
static unsigned int
steal_from_kni(struct rte_mbuf **pkts, unsigned int num, uint8_t port_id) {
	unsigned int i, curr = 0;
	int ret;
	for (i = 0;i < num;i ++) {
		pkts[curr] = pkts[i];
		ret = check_steal(pkts[i], port_id);
		if (ret != 0) {
			curr ++;
		}
	}
...
kni_ingress(struct kni_port_params *p)
{
	...
	for (i = 0; i < nb_kni; i++) {
		...
		// added by zhangpu
		nb_rx = steal_from_kni(pkts_burst, nb_rx, port_id);
		/* Burst tx to kni */
